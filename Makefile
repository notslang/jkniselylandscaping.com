...
BASE_DEPS = tmp/render.js tmp/view/layout.marko.js tmp/page-list.json tmp/slideshow-list.json

public/%/index.html: content/%/index.html tmp/view/normal.marko.js $(BASE_DEPS)
	mkdir -p "$(dir $@)"
	node tmp/render.js "$<" normal < "$<" > "$@"

public/index.html: tmp/view/home.marko.js $(BASE_DEPS)
	mkdir -p "$(dir $@)"
	echo "foo" | node tmp/render.js / home > "$@"
...
tmp/slideshow-list.json: tmp/slideshow-list.js $(INPUT_IMGS) tmp/npm-install-done
	node tmp/slideshow-list.js > "$@.compare"
	if cmp -s "$@.compare" "$@"; then \
		rm "$@.compare"; \
	else \
		mv "$@.compare" "$@"; \
